export async function init(ctx, info) {
  ...
  const mirror_video = info.mirror && player_type.video ? `class="video-mirror"` : ""
  const mirror_controlls = info.mirror && player_type.video ? `
    <style>
  ...
  ctx.handleEvent("buffer", async ([info, buffer]) => {
    try {
      ...
      // jmuxer is null when user opens the same livebook session from multiple tabs.
      if (jmuxer == null) {
        send_error("Kino Player doesn't support multiple clients");
      } else {
        jmuxer.feed({
          video: video,
          audio: audio,
